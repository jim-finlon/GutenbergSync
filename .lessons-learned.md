...
- **2025-12-08**: Fixed WSL rsync discovery bug - returned "wsl rsync" as executable path which failed in ProcessStartInfo. Now returns "wsl" and uses separate method GetRsyncVersionFromWSLAsync with proper argument handling. Version detection works for WSL installations. ProcessStartInfo.FileName must be just the executable name, with arguments in the Arguments field.
